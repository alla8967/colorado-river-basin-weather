...
```text
ml_reconstruction/weather_reconstruction_model/scripts/pipeline/station_selection.py
```
...
```text
ml_reconstruction/weather_reconstruction_model/scripts/pipeline/training_tables.py
```
...
```text
ml_reconstruction/weather_reconstruction_model/scripts/config.py
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_station_validation.py USC00052223
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_station_validation.py USC00052223 --skip-cpp
```

To test a different station:

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_station_validation.py USC00025494
```

To change the number of hubs:

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_station_validation.py USC00052223 --hub-count 8
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_training_table.py USC00052223 5
```

Output:

```text
ml_reconstruction/weather_reconstruction_model/outputs/training_tables/USC00052223_5_hubs.csv
```

### 2. Train The Model

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/train_temperature_model.py ml_reconstruction/weather_reconstruction_model/outputs/training_tables/USC00052223_5_hubs.csv
```

Outputs:

```text
ml_reconstruction/weather_reconstruction_model/outputs/predictions/USC00052223_5_hubs_predictions.csv
ml_reconstruction/weather_reconstruction_model/outputs/validation/USC00052223_5_hubs_actual.csv
ml_reconstruction/weather_reconstruction_model/outputs/validation/USC00052223_5_hubs_predicted.csv
```
...
```text
cpp_scoring_engine/C++_Weather_Station_Proxy_Engine/validate_prediction_similarity.cpp
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/batch_validate_models.py --limit 25
```

Output:

```text
ml_reconstruction/weather_reconstruction_model/outputs/reports/batch_25_targets_5_hubs.csv
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/analyze_batch_results.py ml_reconstruction/weather_reconstruction_model/outputs/reports/batch_100_targets_5_hubs.csv --write-markdown
```
...
```text
ml_reconstruction/weather_reconstruction_model/outputs/reports/batch_100_targets_5_hubs_analysis.md
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_general_training_table.py --limit 25
```

Output:

```text
ml_reconstruction/weather_reconstruction_model/outputs/general_training_tables/general_25_targets_5_hubs.csv
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/train_general_temperature_model.py ml_reconstruction/weather_reconstruction_model/outputs/general_training_tables/general_23_targets_5_hubs.csv
```
...
```text
ml_reconstruction/weather_reconstruction_model/outputs/predictions/general_23_targets_5_hubs_terrain_general_predictions.csv
ml_reconstruction/weather_reconstruction_model/outputs/reports/general_23_targets_5_hubs_terrain_general_station_metrics.csv
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/train_general_temperature_model.py ml_reconstruction/weather_reconstruction_model/outputs/general_training_tables/general_23_targets_5_hubs.csv --exclude-terrain
```

That writes separate comparison files:

```text
ml_reconstruction/weather_reconstruction_model/outputs/predictions/general_23_targets_5_hubs_no_terrain_general_predictions.csv
ml_reconstruction/weather_reconstruction_model/outputs/reports/general_23_targets_5_hubs_no_terrain_general_station_metrics.csv
```
...
```text
ml_reconstruction/weather_reconstruction_model/scripts/train_tree_temperature_model.py
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/train_tree_temperature_model.py ml_reconstruction/weather_reconstruction_model/outputs/general_training_tables/general_23_targets_5_hubs.csv
```

To compare the same table while ignoring terrain features:

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/train_tree_temperature_model.py ml_reconstruction/weather_reconstruction_model/outputs/general_training_tables/general_23_targets_5_hubs.csv --exclude-terrain
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_general_training_table.py --limit 10 --hub-count 5 --target-neighbor-count 10
```
...
```text
ml_reconstruction/weather_reconstruction_model/outputs/general_training_tables/general_8_targets_5_hubs_10_target_neighbors.csv
```

Daily temperature data can be cached in SQLite with:

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_weather_cache.py
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_general_training_table.py --limit 25 --hub-count 5 --target-neighbor-count 10 --use-cache
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_general_training_table.py \
  --limit 25 \
  --hub-count 5 \
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_diagnostics_report.py
```
...
```text
ml_reconstruction/weather_reconstruction_model/outputs/reports/diagnostics/general_97_targets_5_hubs_terrain_standard_random_forest_diagnostics.html
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_model_comparison_report.py \
  --baseline ml_reconstruction/weather_reconstruction_model/outputs/reports/general_23_targets_5_hubs_terrain_quick_random_forest_station_metrics.csv \
  --candidate ml_reconstruction/weather_reconstruction_model/outputs/reports/general_22_targets_5_hubs_10_target_neighbors_terrain_quick_random_forest_station_metrics.csv \
...
```text
ml_reconstruction/weather_reconstruction_model/outputs/reports/comparisons/
```

The older per-station batch validation script also supports the SQLite cache:

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/batch_validate_models.py --limit 25 --hub-count 5 --use-cache
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/batch_validate_models.py \
  --limit 25 \
  --hub-count 5 \
  --use-cache \
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/build_station_terrain_features.py
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/validate_dem_alignment.py
```

Output:

```text
ml_reconstruction/terrain_data/processed/dem_alignment_validation.csv
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_reference_cases.py
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_reference_cases.py --skip-rerun
```

To include the independent C++ validator for both reference cases:

```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/run_reference_cases.py --with-cpp
```
...
```bash
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/tests/test_common_utils.py
.venv/bin/python ml_reconstruction/weather_reconstruction_model/scripts/tests/test_pipeline_utils.py
```
