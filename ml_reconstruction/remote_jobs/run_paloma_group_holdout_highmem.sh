...
#SBATCH --array=1-5
#SBATCH --output=ml_reconstruction/remote_jobs/logs/paloma-group-hm.%A_%a.out
#SBATCH --error=ml_reconstruction/remote_jobs/logs/paloma-group-hm.%A_%a.err
...
GROUP_FILE="ml_reconstruction/weather_reconstruction_model/outputs/paloma/station_holdout_groups/${VARIABLE}/${GROUP_ID}.csv"
...
mkdir -p "$DURABLE_PROJECT_DIR/ml_reconstruction/weather_reconstruction_model/outputs/paloma/station_holdout_groups/${VARIABLE}"
rsync -av \
...
rsync -av \
  "ml_reconstruction/weather_reconstruction_model/outputs/paloma/station_holdout_groups/${VARIABLE}/" \
  "$DURABLE_PROJECT_DIR/ml_reconstruction/weather_reconstruction_model/outputs/paloma/station_holdout_groups/${VARIABLE}/"
