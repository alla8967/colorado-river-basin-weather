...
```text
ml_reconstruction/weather_reconstruction_artifacts/raw_dem_archive/
ml_reconstruction/weather_reconstruction_artifacts/noaa_year_archive/
ml_reconstruction/weather_reconstruction_artifacts/archive/
ml_reconstruction/weather_reconstruction_artifacts/scratch/
Raw_DEM/
...
```text
.venv/
web_app/station-proxy-backend/.venv/
__pycache__/
*.pyc
```
...
rsync -av \
  ...
  --exclude "*.pyc" \
  --exclude ".DS_Store" \
  --exclude "web_app/station-proxy-backend/.venv/" \
  --exclude "ml_reconstruction/weather_reconstruction_artifacts/" \
  --exclude "ml_reconstruction/weather_reconstruction_model/outputs/" \
  --exclude "Raw_DEM/" \
  --exclude "ml_reconstruction/NOAA_Inventory_Sort/NOAA_GHCN_ByYear/" \
...
```bash
rsync -av \
  "Colorado River Basin Project/ml_reconstruction/weather_reconstruction_model/cache/weather_data.sqlite" \
  YOUR_IDENTIKEY@login.rc.colorado.edu:/projects/YOUR_IDENTIKEY/crb_weather_project/ml_reconstruction/weather_reconstruction_model/cache/
```
...
```bash
sbatch ml_reconstruction/remote_jobs/run_smoke.sh
```
...
pwd
ls
ls ml_reconstruction/NOAA_Inventory_Sort/*_app_ready.csv
ls ml_reconstruction/terrain_data/processed/station_terrain_features.csv
ls ml_reconstruction/weather_reconstruction_model/cache/weather_data.sqlite
ls ml_reconstruction/remote_jobs/run_smoke.sh
```
...
```bash
rsync -av \
  YOUR_IDENTIKEY@login.rc.colorado.edu:/scratch/alpine/YOUR_IDENTIKEY/crb_weather_runs/current/ml_reconstruction/weather_reconstruction_model/outputs/ \
  "Colorado River Basin Project/ml_reconstruction/weather_reconstruction_model/outputs/"
```
