...
```text
weather_reconstruction_artifacts/raw_dem_archive/Raw_DEM/*.tif
weather_reconstruction_artifacts/noaa_year_archive/NOAA_GHCN_ByYear/*.csv.gz
```
...
```text
weather_reconstruction_model/cache/*_pairwise_skill_train_through_*.csv
weather_reconstruction_model/outputs/general_training_tables/
weather_reconstruction_model/outputs/predictions/
weather_reconstruction_model/outputs/reports/
```
...
```bash
python weather_reconstruction_model/scripts/check_remote_environment.py --require-rasterio
```
...
```bash
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset smoke
```

For build presets, the runner first creates a pairwise historical skill CSV in
`weather_reconstruction_model/cache/`, then passes that file into
...
```bash
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset medium
```

Full run:

```bash
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset full
```

Wider remote experiments:

```bash
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset wide-medium
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset wide-large
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset wide-full
```
...
```bash
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset holdout-full --general-table weather_reconstruction_model/outputs/general_training_tables/option_c_limit97_5_hubs_10_target_neighbors_multiscale_terrain.csv
```

Preview commands without running them:

```bash
python weather_reconstruction_model/scripts/run_remote_pipeline.py --preset medium --dry-run
```
