...
This project creates large model inputs, predictions, diagnostics, and smoke-test outputs. Keep source code in `weather_reconstruction_model/`; keep bulky generated results out of the code path whenever possible.

## Active Outputs

`weather_reconstruction_model/outputs/` is for the current working run: the latest training table, current predictions, current station metrics, and HTML reports you are actively reviewing.
...
`weather_reconstruction_model/model_runs/<model_run_id>/` is the stable handoff
...
`../weather_reconstruction_artifacts/archive/` is for older runs that are still worth preserving for comparison, but are not the current working target.

Examples include older full 97-target tables, previous option-C tables, and old validation CSVs.

## Scratch Outputs

`../weather_reconstruction_artifacts/scratch/` is for smoke tests, small experiments, and exploratory reports. These files are useful for debugging but should not be treated as canonical results.
